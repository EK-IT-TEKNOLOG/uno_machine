...
class PortExp_MCP23S08():
    ...
    def __init__(self, spi_bus, pin_cs, addr = 0):
        ...
        self.pin_cs = pin_cs#Pin(pin_cs, Pin.OUT)  # Create the SS/CS (L) pin
        self.addr = addr
    ...
    def gp_direction(self, gp, val):
        if gp < 0 or gp > 7 or (val != self.OUTPUT and val != self.INPUT): # Check the input parameters
            return
        
        reg_val = self.read_register(self.IODIR) # Read the current status of the register
        if val == self.OUTPUT:                   # Direction is output
            reg_val &= ~(1 << gp)
        else:                               # Direction is input
            reg_val |= 1 << gp
        self.write_register(self.IODIR, reg_val) # Write the updated value of the register
    ...
    def gp_interrupt(self, gp, val):
        ...
        reg_val = self.read_register(self.GPINTEN) # Read the current status of the register
        if val == self.OFF:                      # Interrupt disabled on GPIO pin
            reg_val &= ~(1 << gp)
        else:                               # Interrupt is enabled on GP pin
            reg_val |= 1 << gp
        self.write_register(self.GPINTEN, reg_val) # Write the updated value of the register
    ...
    def gp_pullup(self, gp, val):
        ...
        reg_val = self.read_register(self.GPPU)  # Read the current status of the register
        if val == self.OFF:                      # No pull-up on GPIO pin
            reg_val &= ~(1 << gp)
        else:                               # Pull-up is enabled on GPIO pin
            reg_val |= 1 << gp
        self.write_register(self.GPPU, reg_val)  # Write the updated value of the register        
    ...
    def gp_get_value(self, gp):
        ...
        return (self.read_register(self.GPIO) >> gp) & 1 # Read the current status, shift and mask out
        

    def gp_set_value(self, gp, val):
        if gp < 0 or gp > 7 or (val != self.OFF and val != self.ON): # Check the input parameters
            return        
        
        reg_val = self.read_register(self.GPIO)  # Read the current status of the register
        if val == self.OFF:                      # Set GP pin low
            reg_val &= ~(1 << gp)
        else:                               # Set GP pin high
            reg_val |= 1 << gp
        self.write_register(self.GPIO, reg_val)  # Write the updated value of the register
